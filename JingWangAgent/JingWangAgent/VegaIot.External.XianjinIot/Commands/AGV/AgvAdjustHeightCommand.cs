...
internal class AgvAdjustHeightCommand : SimpleCommand<DefaultAgv>
{
    ...
    public override async Task<DeviceServiceInvokeResponse> Invoke(DeviceServiceInvokeRequest deviceServiceInvokeRequest)
    {
        ...
        deviceServiceInvokeRequest.Params["PlcCommand"] = "adjust_height";

        var deviceOperationResponse = await InteractingDevice.PlcOpertaion(deviceServiceInvokeRequest);
        var errormsg = string.Format($"adjust_height：{deviceOperationResponse.Code}_{deviceOperationResponse.Message}");
    ...
    private async Task CommandCallback(DeviceServiceInvokeResponse targetResponse)
    {
        ...
        await _mqttClient.PublishBinaryAsync("jlc/mes/report/agv/adjust_height_ack", JsonSerializer.Serialize(agvAdjustHeightAckPayload).GetBytes(), MQTTnet.Protocol.MqttQualityOfServiceLevel.ExactlyOnce);
    }

    private async Task CommandReport(DeviceServiceInvokeResponse targetResponse, int status)
    {
        var agvWalkLocationPayload = new AgvAdjustHeightReportPayload()
        {
            header = new HeaderEntity()
            {
                signature = _xianJinIotOptions.Signature,
        ...
        await _mqttClient.PublishBinaryAsync("jlc/mes/report/agv/adjust_height_reoprt", JsonSerializer.Serialize(agvWalkLocationPayload).GetBytes(), MQTTnet.Protocol.MqttQualityOfServiceLevel.ExactlyOnce);
    }
}
