...
internal class AgvLoadSiloCommand : SimpleCommand<DefaultAgv>
{
    ...
    private async Task CommandCallback(DeviceServiceInvokeResponse targetResponse)
    {
        ...
        await _mqttClient.PublishBinaryAsync("jlc/mes/report/agv/load_silo_ack", JsonSerializer.Serialize(agvAdjustHeightAckPayload).GetBytes(), MQTTnet.Protocol.MqttQualityOfServiceLevel.ExactlyOnce);
    }
    ...
    private async Task CommandReport(DeviceServiceInvokeResponse targetResponse, int status)
    {
        ...
        await _mqttClient.PublishBinaryAsync("jlc/mes/report/agv/load_silo_reoprt", JsonSerializer.Serialize(agvWalkLocationPayload).GetBytes(), MQTTnet.Protocol.MqttQualityOfServiceLevel.ExactlyOnce);
    }
}
