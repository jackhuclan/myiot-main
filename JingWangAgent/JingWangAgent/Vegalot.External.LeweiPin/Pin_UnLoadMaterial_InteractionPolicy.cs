...
{
    public class Pin_UnloadMaterial_InteractionPolicy : AbstractUnloadMaterialInteractionPolicy<Pin>
    {
        private readonly ILogger<Pin_UnloadMaterial_InteractionPolicy> logger;

        public Pin_UnloadMaterial_InteractionPolicy(
            ILogger<Pin_UnloadMaterial_InteractionPolicy> logger,
        ...
        public override async Task<DeviceServiceInvokeResponse> CompleteUnloadMaterial(DeviceServiceInvokeRequest deviceServiceInvokeRequest)
        {
            ...
            {
                //InteractingDevice.modbusIpMaster.WriteSingleRegister(1, position == 1 ? (ushort)27 : (ushort)37, 1);
                ...
                InteractingDevice.mitsubishiClient?.Write(position == 1 ? "W105" : "W115", (short)0);//停止读取层数以及其他信息
                InteractingDevice.mitsubishiClient?.Write(position == 1 ? "W106" : "W116", (short)1);//通知上PIN机料箱已运走
                await Task.Delay(1500);//做延迟，解决W105/W115 还未执行完就执行清空料仓数据，导致最后一行数据没有清除掉
