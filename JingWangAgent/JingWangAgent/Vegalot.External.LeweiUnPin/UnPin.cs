...
public class UnPin : Device
{
    ...
    public Task KeepConnect()
    {
        return Task.Run(async () =>
        {
            ...
            if (mitsubishiClient == null || mitsubishiClient.Connected == false)
            {
            ...
            try
            {
                var plcValue = mitsubishiClient.ReadUInt16("0").Value;
    ...
    public async Task<bool> WritePlc(CurrentContext currentContext)
    {
        ...
        {
            case 3:
                mitsubishiClient.Write(currentContext.PlcAddr, currentContext.PlcValue.ToUshort());
                break;
            ...
            case 4:
                ...
                mitsubishiClient.Write(currentContext.PlcAddr, writeVal.Length);
                break;

            case 5:
                var value = currentContext.PlcValue;
                //  ushort[] val = new ushort[2] { 6864, 1 };
                logger.LogDebug($"WritePlc: {System.Text.Json.JsonSerializer.Serialize(value)}");
                mitsubishiClient.Write(currentContext.PlcAddr, value);
    ...
    public async Task<bool> ResetSignal(int type)
    {
        ...
        {
            case 1:
                mitsubishiClient?.Write("46", 1);
                return true;

            case 2:
                mitsubishiClient?.Write("45", 1);
                return true;

            case 3:
                mitsubishiClient?.Write("47", 1);
                return true;

            case 4:
                mitsubishiClient?.Write("56", 1);
                return true;

            case 5:
                mitsubishiClient?.Write("55", 1);
                return true;

            case 6:
                mitsubishiClient?.Write("57", 1);
                return true;
        }
