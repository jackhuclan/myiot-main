...
namespace Vegalot.External.LeweiUnPin
{
    public class UnPinPropertyHandler : AbstractPropertyHandler<UnPin>
    {
        ...
        public override void CollectPropertyValues()
        {
            ...
            if (InteractingDevice.mitsubishiClient != null && InteractingDevice.mitsubishiClient.Connected)
            {
                try
                {
                    //var triggerSignal = InteractingDevice.mitsubishiClient.Read("W50",ushort.MaxValue).Value;
                    ...
                    realProperties["HandleLeftAskUpload"] = 0;
                    realProperties["HandleLeftPanelFinish"] = 0;
                    realProperties["HandleLeftAskDownLoad"] = 0;
                    realProperties["HandleRightAskUpload"] = 0;
                    realProperties["HandleRightPanelFinish"] = 0;
                    realProperties["HandleRightAskDownLoad"] =0;
                    //realProperties["PlcItemNo"] = triggerSignal.ToList().Skip(20).Take(20).ToArray().ByteArrayToString().Replace("\0", "").Replace("\r", "");
                }
                catch (Exception ex)
                {
