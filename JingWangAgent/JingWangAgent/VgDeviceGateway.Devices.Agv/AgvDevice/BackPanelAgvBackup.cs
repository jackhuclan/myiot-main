...
{
    public class BackPanelAgvBackup : DeviceShare<DefaultAgv>, IAgvDevice
    {
        private readonly ILogger<BackPanelAgvBackup> logger;
        public readonly byte slaveId;

        public BackPanelAgvBackup(ILogger<BackPanelAgvBackup> logger, IServiceProvider serviceProvider, DefaultAgv device) : base(serviceProvider, device)
        {
            this.logger = logger;
        ...
        private async Task<bool> SetPlcSingleSpindleAsync(DeviceServiceInvokeRequest deviceServiceInvokeRequest, SwapPanel item, int plcnextbehavior = 0)
        {
            ...
            if (behavior == 1)
            {
                ...
                if (panel != null)
                {
                    ...
                    {
                        panel.PanelLength = await GetPanthLength(panel);
                    }
        ...
        private async Task<float> GetPanthLength(Panel panel)
        {
            float panthLength = 0;
            try
            {
                if (panel.PanelLength <= 0)
                {
                    ...
                    {
                        var messagea = $"从中控获取板长条件：物料号：{panel.ItemCode},板长：{panel.PanelLength}，配置GetItemInfo：{DeviceDescriptor.Extra["GetItemInfo"].ToStr()}";
                    ...
                    {
                        var messageb = $"从中控获取板长条件不满足！！！：物料号：{panel.ItemCode},板长：{panel.PanelLength}，是否存在配置GetItemInfo：{DeviceDescriptor.Extra.ContainsKey("GetItemInfo")}";
