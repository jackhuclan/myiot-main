...
{
    public class FrontToolAgvStateHandler : DeviceShare<DefaultAgv>, IAgvStateHandler
    {
        private readonly ILogger<FrontToolAgvStateHandler> logger;

        public FrontToolAgvStateHandler(ILogger<FrontToolAgvStateHandler> logger, IServiceProvider serviceProvider, DefaultAgv device) : base(serviceProvider, device)
        {
            this.logger = logger;
        }
        ...
        public void UpdatePLcSiloInfo()
        {
            try
            {
                ...
                {
                    logger.LogError($"PLC链接失败： modbusIpMaster 为 NULL");
                    return;
                }
                ushort[] valuedata = new ushort[InteractingDevice.PayloadCutterTrays.Count];
                for (int i = 0; i < InteractingDevice.PayloadCutterTrays.Count; i++)
                {
                    //PLC料仓信息
                    var panelType = 0;
                    var panelinfo = InteractingDevice.PayloadCutterTrays[i];
                    if (panelinfo != null && panelinfo.Status == CutterTrayStatus.New)
                    {
                        panelType = 1;
                    }
                    else if (panelinfo != null && panelinfo.Status == CutterTrayStatus.Old)
                    {
                        panelType = 2;
                    }
                    else
                ...
                InteractingDevice.modbusIpMaster.WriteMultipleRegisters(InteractingDevice.slaveId, 3030, valuedata);
            }
            catch (Exception ee)
            {
