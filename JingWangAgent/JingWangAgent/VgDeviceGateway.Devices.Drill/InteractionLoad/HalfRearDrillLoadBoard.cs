...
{
    public class HalfRearDrillLoadBoard : DeviceShare<DefaultDrill>, IDrillLoadInteraction
    {
        private readonly ILogger<HalfRearDrillLoadBoard> logger;
        private readonly byte slaveID;
        private readonly bool codeReaderOnOff;

        public HalfRearDrillLoadBoard(ILogger<HalfRearDrillLoadBoard> logger, IServiceProvider serviceProvider, DefaultDrill device) : base(serviceProvider, device)
        {
            this.logger = logger;
        ...
        public async Task<DeviceServiceInvokeResponse> CompleteLoadMaterialLocal(DeviceServiceInvokeRequest deviceServiceInvokeRequest, int position)
        {
            ...
            logger.LogDebug($"CompleteLoadMaterial 机器【{InteractingDevice.DeviceDescriptor.DeviceName}】  人员结束上下料");
