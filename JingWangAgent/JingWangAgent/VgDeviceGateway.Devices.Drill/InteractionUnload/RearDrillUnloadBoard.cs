...
{
    public class RearDrillUnloadBoard : DeviceShare<DefaultDrill>, IDrillUnloadInteraction
    {
        private readonly ILogger<RearDrillUnloadBoard> logger;
        public readonly byte slaveID;

        public RearDrillUnloadBoard(ILogger<RearDrillUnloadBoard> logger, IServiceProvider serviceProvider, DefaultDrill device) : base(serviceProvider, device)
        {
            this.logger = logger;
        ...
        public async Task<DeviceServiceInvokeResponse> CompleteUnloadMaterialLocal(DeviceServiceInvokeRequest deviceServiceInvokeRequest, int position)
        {
            if (deviceServiceInvokeRequest.Params != null && deviceServiceInvokeRequest.Params.ContainsKey("IsLastStep") && deviceServiceInvokeRequest.Params["IsLastStep"].ToBool())
            {
                if (InteractingDevice.isCallAgvUnload)
                {
                    ...
                    return await InteractingDevice.Response(ErrorCodes.Sys.SUCCESS, $" 单个agv 只下料动作结束84->{DeviceDescriptor.DeviceId}", deviceServiceInvokeRequest.Params);
                }
                else
                {
                    InteractingDevice.allowAllAgv = true;
            ...
            else
            {
                ...
                if (isfinished[0] != 1)
                {
                    return await InteractingDevice.Response(ErrorCodes.Sys.FAIL, $"buffer下料未完成84->{DeviceDescriptor.DeviceId}", deviceServiceInvokeRequest.Params);
                }
                ...
                logger.LogWarning($"CompleteUnloadMaterial 下料完成未修改的 【 PayloadPanels 】 {JsonSerializer.Serialize(InteractingDevice.PayloadPanels)}");
                ...
                logger.LogWarning($"CompleteUnloadMaterial 下料完成 UnloadingPanel 赋值：{JsonSerializer.Serialize(deviceServiceInvokeRequest!.Params["UnloadingPanel"])}");
                ...
                logger.LogWarning($"CompleteUnloadMaterial 下料完成后 【 PayloadPanels 】 {JsonSerializer.Serialize(InteractingDevice.PayloadPanels)}");
        ...
        public async Task<DeviceServiceInvokeResponse> PrepareUnloadMaterialLocal(DeviceServiceInvokeRequest request, int position)
        {
            ...
            return await InteractingDevice.Response(result ? ErrorCodes.Sys.SUCCESS : ErrorCodes.Sys.FAIL,
                              result ? string.Empty : $"机器尚未准备好:buffer准备状态 {readyFlag},上电状态：{isPower}, 自动模式:{model} 非报警状态:{isNoFalt} 非急停状态:{isNoFalt},buffer熟料层 轴：{position}有板子状态：{hasBoard}  buffer是否定位完成{isPosition}->84->{DeviceDescriptor.DeviceId}");
        }
                                                                                 ...
                                                                                 string methodName = "")
        {
            ...
            if (deviceServiceInvokeRequest.Params == null || !deviceServiceInvokeRequest.Params.ContainsKey("Position"))
            {
                ...
                return await this.InteractingDevice.Response(ErrorCodes.Sys.FAIL, $"{methodName} 传入的参数不包含位置信息84->{DeviceDescriptor.DeviceId}");
            }
            ...
            if (!(position > 0 && position <= this.InteractingDevice.spindleNum))
            {
                ...
                eventMessage = $"{methodName} 下发Position参数值{position}不在1和{this.InteractingDevice.spindleNum}之间，请下发正确的上料轴信息84->{DeviceDescriptor.DeviceId}";
