             ...
             .TriggerAlways(async () =>
             {
                 try
                 {
                     if (WatchingProperties.Property("HandleLeftPanelFinish").NewValue.ToInt() == 1)
                     {
                         ...
                         logger.LogInformation($"HandleLeftPanelFinish: 原始值: panelLength:{JsonSerializer.Serialize(InteractingDevice.modbusIpMaster.ReadHoldingRegisters(1, 80, 2))},panelWidth:{JsonSerializer.Serialize(InteractingDevice.modbusIpMaster.ReadHoldingRegisters(1, 82, 2))}, PinOffset:{JsonSerializer.Serialize(InteractingDevice.modbusIpMaster.ReadHoldingRegisters(1, 84, 2))},StackCount:{InteractingDevice.modbusIpMaster.ReadHoldingRegisters(1, 86, 2)}");
