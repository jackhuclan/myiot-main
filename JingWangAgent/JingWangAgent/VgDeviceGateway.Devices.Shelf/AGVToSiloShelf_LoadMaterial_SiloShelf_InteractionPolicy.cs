...
public class AGVToSiloShelf_LoadMaterial_SiloShelf_InteractionPolicy : AbstractLoadMaterialInteractionPolicy<SiloShelf>
{
    private readonly ILogger<AGVToSiloShelf_LoadMaterial_SiloShelf_InteractionPolicy> logger;

    public AGVToSiloShelf_LoadMaterial_SiloShelf_InteractionPolicy(ILogger<AGVToSiloShelf_LoadMaterial_SiloShelf_InteractionPolicy> logger,
    ...
    public override async Task<DeviceServiceInvokeResponse> PrepareLoadMaterial(DeviceServiceInvokeRequest deviceServiceInvokeRequest)
    {
        ...
        {
            return await InteractingDevice.Locations[$"{position}"].PrepareLoadMaterial(deviceServiceInvokeRequest);
        }, "PrepareLoadMaterial");
    }
    ...
    public override async Task<DeviceServiceInvokeResponse> InvokeLoadMaterial(DeviceServiceInvokeRequest deviceServiceInvokeRequest)
    {
        return await DoService(deviceServiceInvokeRequest, async (request, position) =>
        {
            return await InteractingDevice.Locations[$"{position}"].InvokeLoadMaterial(deviceServiceInvokeRequest);
        }, "InvokeLoadMaterial");
    }
    ...
    public override async Task<DeviceServiceInvokeResponse> CompleteLoadMaterial(DeviceServiceInvokeRequest deviceServiceInvokeRequest)
    {
        return await DoService(deviceServiceInvokeRequest, async (request, position) =>
        {
            return await InteractingDevice.Locations[$"{position}"].CompleteLoadMaterial(deviceServiceInvokeRequest);
        ...
        string methodName = "")
    {
        ...
        try
        {
            ...
            if (deviceServiceInvokeRequest.Params.ContainsKey("IsFullSilo"))
            {
                logger.LogInformation($"{methodName} 下发IsFullSilo参数值{deviceServiceInvokeRequest.Params["IsFullSilo"].ToBool()}");
            }
        ...
        {
            logger.LogError(ex, $"{position}号料架上料异常：{ex.Message}");
            eventId = $"{methodName}_Exception";
            eventName = "Exception";
            eventMessage = ex.Message;
            return await InteractingDevice.Response(ErrorCodes.Sys.EXCEPTION_CODE, eventMessage);
        }
        finally
        {
