...
namespace VgDeviceGateway.Devices.UnPin
{
    public class UnPin_LoadMaterial_InteractionPolicy : AbstractLoadMaterialInteractionPolicy<UnPin>
    {
        ...
        public override async Task<DeviceServiceInvokeResponse> CompleteLoadMaterial(DeviceServiceInvokeRequest deviceServiceInvokeRequest)
        {
            ...
            //InteractingDevice.modbusIpMaster.WriteSingleRegister(1, spli == 1 ? (ushort)46 : (ushort)56, 1);
            ...
            return await DoService(deviceServiceInvokeRequest, async (request, position) =>
            {
                ...
                else
                {
                  ...
                  , new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                    ...
                    {
                        //if (operationEntity.PanelList == null || operationEntity.PanelList.Count == 0)
                ...
                InteractingDevice.modbusIpMaster?.WriteSingleRegister(1, position == 1 ? (ushort)46 : (ushort)56, 1);
                var pcs = InteractingDevice.PayloadPanels.FirstOrDefault(p => p.Pcs > 0)?.Pcs;
                InteractingDevice.modbusIpMaster?.WriteMultipleRegistersAsync(1, 86, pcs.ToFloat().StringFloatToUshort());
