...
export default {
  name: "WorkStationAndRouteSelect", 
  data() {
    return {
      showFlag: false,
      // 遮罩层
  ...
  methods: {
    ...
    getList(isSearch) {
      this.loading = true;
      getRoutesByProcess(this.queryParams).then((res) => {
        ...
        // 多选框回显
        this.$nextTick((res) => {
          const showDataId = this.showData.map((v) => v.id);
    ...
    rowDblclick(row, column, event) {
      ...
      //找到选中的行
      if (findRow) {
        this.showData = this.showData.filter((v) => v.id != row.id);
    ...
    handelSelectAll(selection) {
      if (this.getIsAllChecked()) {
        ...
        this.routeList.forEach((v) => {
          if (!showDataId.includes(v.id)) {
            this.showData.push(v);
          }
        });
      } else {
        const routeId = this.routeList.map((v) => v.id);
        this.showData = this.showData.filter((v) => !routeId.includes(v.id));
      }
    },
    // 单选
