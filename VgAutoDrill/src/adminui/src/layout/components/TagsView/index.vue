...
export default {
  ...
  methods: {
    ...
    addTags() {
      ...
      if (name && !this.visitedViews.some((v) => v.title == title)) {
      ...
      this.$nextTick(() => {
        for (const tag of tags) {
          if (
            ...
            tag.to.meta.title == this.$route.meta.title
          ) {
    ...
    closeLeftTags() {
      this.$tab.closeLeftPage(this.selectedTag).then((visitedViews) => {
        const isEqualFullPath = visitedViews.find(
          (i) => i.fullPath === this.$route.fullPath
        );
        const isEqualFullTitle = visitedViews.find(
          (i) => i.title === this.$route.meta.title
        );
        if (!isEqualFullPath && !isEqualFullTitle) {
          this.toLastView(visitedViews);
        }
      });
    },
