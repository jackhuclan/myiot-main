<template>
  ...
  >
    <div slot="title" class="drawer_title">
      <p>
        <span>调度记录Id--{{ detailTitle.id }} </span>
        <i
          v-if="isRefresh"
        ...
        <el-collapse-item title="日志" name="2">
          ...
          >
            <li v-for="(item, index) in schedulementDetails" :key="index">
              <p>{{ item.createTime }}</p>
              <p><pre class="pre"><span v-if="typeof item.spindle =='number'">第{{ item.spindle }}轴：</span>{{ item.message }}</pre></p>
            </li>
          </div>
          <div v-else style="padding: 5px;font-size: 16px;margin-bottom: 10px;">暂无数据</div>
        </el-collapse-item>
      </el-collapse>
    </div>
  </el-drawer>
...
import { getDetailData as scheduleHistoryGetDetailData } from "@/api/device/scheduleHistory";
...
export default {
  ...
  methods: {
    ...
    getDetail(id) {
      ...
      switch (this.isPage) {
        case "schedulement":
          api = schedulementGetDetailData;
          break;
        case "scheduleHistory":
          api = scheduleHistoryGetDetailData;
          break;
        default:
          api = getDetailData;
          break;
