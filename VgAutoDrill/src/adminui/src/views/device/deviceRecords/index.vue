<template>
  <div class="app-container">
    ...
    >
      ...
      <!-- 原label:结束到开始总时间;因不是实时数据,暂时先隐藏该字段-->
      <!-- <el-table-column
        label="上趟打板结束到下趟打板开始间隔时长"
        align="center"
        min-width="280"
        key="endToStartTime"
        prop="endToStartTime"
      ...
      <el-table-column
        label="Buffer上板完成到开始打板时长"
        min-width="210"
        align="center"
        key="bufferRawExistToRunTime"
        prop="bufferRawExistToRunTime"
      ...
      <el-table-column
        ...
        min-width="180"
        align="center"
        key="bufferRawCompleteTime"
        prop="bufferRawCompleteTime"
        v-if="columns[15].visible"
      >
      </el-table-column>
      <el-table-column
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { key: 12, label: "刀具寿命", visible: true },
        { key: 13, label: "清洗夹头总时长", visible: true },
        { key: 14, label: "无钻带参数时长", visible: true },
        { key: 15, label: "上料时长", visible: false },
        { key: 16, label: "等待上料时长", visible: false },
        { key: 17, label: "数据日期", visible: true },
        { key: 18, label: "中控记录时间", visible: true },
      ],
    };
  },
  activated() {
  ...
  methods: {
    ...
    async getList(isSearch) {
      ...
      this.queryParams.queryOrderBy =
        ...
        this.$cache.local.get("deviceRecords_QueryOrderBy") != "0"
          ...
          : 2;
      const local_arr = this.$cache.local.getJSON("deviceRecords_routeCodeList")
        ? JSON.parse(this.$cache.local.getJSON("deviceRecords_routeCodeList"))
        : [];
    ...
    resetQuery() {
      // 重置时时间范围也重置
      this.$cache.local.remove("deviceRecords_routeCodeList");
    ...
    handleExport() {
      ...
      this.exportExcel(
        "/v1/DeviceRecords/DownLoadSummaryList",
        name,
        this.queryParams
      );
    },
