<template>
  <div class="app-container">
    ...
    <el-collapse v-model="activeNames" class="my-collapse">
      <el-collapse-item name="拆PIN线边仓 ">
        <span class="collapse-title" slot="title">拆PIN线边仓 </span>
        <el-table style="width: 100%" :ref="page" :data="drilledList" border>
          <el-table-column
          ...
          <el-table-column
            show-overflow-tooltip
            label="本托盘数量"
            prop="drilledMaterialCount"
          />
        </el-table>
      </el-collapse-item>
      <el-collapse-item name="熟料缓冲区">
        <span class="collapse-title" slot="title">熟料缓冲区 </span>
        <el-table
          style="width: 100%"
          :ref="page"
          :data="drilledBufferList"
          border
        >
          <el-table-column
            show-overflow-tooltip
      ...
      <el-collapse-item name="中转区">
        <span class="collapse-title" slot="title">中转区</span>
        <el-table
          style="width: 100%"
          :ref="page"
      ...
      <el-collapse-item name="钻机">
        <span class="collapse-title" slot="title">钻机</span>
        <el-table style="width: 100%" :ref="page" :data="drillList" border>
          <el-table-column
            show-overflow-tooltip
            label="批次号"
            prop="itemCode"
...
<script>
import { listVegaDrilledMaterial } from "@/api/masterData/vegaDrilledMaterial";
export default {
  // 维嘉熟料
  name: "VegaDrilledMaterial",
  data() {
    return {
      page: "vegaDrilledMaterial",
      showSearch: true,
      // 拆PIN线边仓
      drilledList: [],
      //熟料缓冲区
      ...
      list: Array.from({ length: 40 }, (v, k) => {
        return {
          id: k,
          // 1、拆PIN线边仓 2、熟料缓冲区, 3、转入途中 4、中转区 5、AGV 6、钻机
          siloCode: "siloCode" + k,
          itemCode: "itemCode" + k,
          locationCode: [1, 2, 3, 4, 5, 6][Math.floor(Math.random() * 6)],
          drilledMaterialCount: Math.floor(Math.random() * 100) + 1,
        };
      }),
    };
  },
  ...
  methods: {
    ...
    // 1-拆PIN线边仓
    getDrilledList() {
      listVegaDrilledMaterial({
        ...this.queryParams,
        location: 1,
      }).then((res) => {
        this.drilledList = res.data;
      });
    },
    // 熟料缓冲区
    getDrilledBufferList() {
      listVegaDrilledMaterial({
        ...this.queryParams,
        location: 2,
      }).then((res) => {
        this.drilledBufferList = res.data;
      });
    },
    // 转入途中
    getTransferInList() {
