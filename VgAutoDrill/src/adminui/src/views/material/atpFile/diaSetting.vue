      ...
      >
        ...
        <div>
          <div
            ...
            :key="'刀具' + index"
          >
            <span class="bg_index" :style="{ fontSize: setFontSize }">{{
              index + 1
            }}</span>
...
import { getItemAtpFile } from "@/api/material/atpFile";
export default {
  name: "DiaSetting",
  components: {
    cutterSelect,
  ...
  computed: {
    ...
    // 背景字体大小
    setFontSize() {
      if (this.form.columnsLimit == 1 && this.form.rowsLimit == 1) {
        return "0px";
      } else if (this.form.columnsLimit < 3 || this.form.rowsLimit < 3) {
        return "40px";
      }
      return "120px";
    },
  ...
  watch: {
    // 判断是否全选
    selectIdList(val) {
      if (val.length == this.multiList.flat(Infinity).length) {
  ...
  methods: {
    getForm() {
      const ItemAtpFileId = localStorage.getItem("diaSettingId");
    ...
    // 获取数据
    getList() {
      this.queryParams.itemAtpFileId = localStorage.getItem("diaSettingId");
    ...
    openCutterSelect() {
      ...
      this.$refs.cutterSelect.getList();
    },
    // 点击刀具选择
    ...
    selectAll() {
      ...
      if (this.isSelectAll) {
        this.multiList.flat(Infinity).map((v) => {
          v.color = "#ccc";
          v.checked = true;
          this.selectList.push(v.orderNum);
          this.selectList = [...new Set(this.selectList)].sort((a, b) => a - b);
    ...
    // 关闭页面
    closePage() {
      const obj = { path: "/material/atpFile" };
