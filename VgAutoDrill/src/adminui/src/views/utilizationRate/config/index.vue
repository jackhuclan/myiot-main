<template>
          ...
          <el-col>
            <el-form-item :label="item.remark + ' :'" class="item">
              <!-- <input-number
                ...
                :min="1"
                :dis="hasPermi(['utilizationRate:config:edit'])"
              /> -->
              ...
              <el-select
                ...
                @change="handleSelect"
                placeholder="请选择"
                :disabled="hasPermi(['utilizationRate:config:edit'])"
              >
                <el-option
          ...
          <el-col>
            <el-form-item class="item">
              <el-button
                ...
                type="primary"
                @click="onSubmit"
                :disabled="hasPermi(['utilizationRate:config:save'])"
                >保存</el-button
              >
              <el-button
                v-else
  ...
  <script>
import {
  getDeviceMaintenanceConfigs,
  saveDeviceMaintenanceConfigs,
} from "@/api/utilizationRate/config";
export default {
  name: "QuickConfig",
  data() {
    return {
  ...
  methods: {
    ...
    getList() {
      getDeviceMaintenanceConfigs().then((res) => {
    ...
    onSubmit() {
      ...
      saveDeviceMaintenanceConfigs(list).then((res) => {
        if (res.code == 0) {
          this.getList();
