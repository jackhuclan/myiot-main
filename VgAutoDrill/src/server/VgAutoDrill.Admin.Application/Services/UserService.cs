...
namespace VgAutoDrill.Admin.Application.Services
{
    public class UserService : BaseService, IUserService
    {
        ...
        public async Task<ResponseDto<GetInfoDto>> GetInfo()
        {
            ...
            {
                return new ResponseDto<GetInfoDto>();
            }

            string depatmentName = string.Empty;
            if (userInfoModel.DepartmentId.HasValue)
            {
                var depatmentModel = await _departmentDomainService.QueryByID(userInfoModel.DepartmentId);
                if (depatmentModel != null)
                {
                    depatmentName = depatmentModel.DepartmentName;
                }
            }

            var roleModel = await _roleDomainService.GetRolesByUserID(UserId);

            var roles = new List<string>();
            var permissions = new List<string>();
            if (userInfoModel.IsAdmin == 1 || (roleModel != null && roleModel.Id == 1))
            {
                roles.Add("admin");
                permissions.Add("*:*:*");
            }
            else
            {
                ...
                if (isExit && roleModel != null && roleModel.Id != 1)
                {
                    permissions = await _menuAuthDomainService.GetMenuAuthData(appCode, roleModel.Id);
                }
            }
        ...
        public async Task<ResponseDto<string>> Login(LoginReq loginReq)
        {
            ...
            {
                return Fail<string>("账号已停用，请联系管理员");
            }
        ...
        public async Task<ResponseDto<string>> LoginForDebug(LoginReq loginReq)
        {
            var pwd = SecurityHelper.SingMd5(loginReq.Password);
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<ResponseDto<UserDetailDto>> QueryByID(long id)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<ResponseDto<string>> UpdateSimple(UpdateSimpleUserReq req)
        {
            var entity = await _userDomainService.QueryByID(req.Id);
            if (entity == null)
            {
        ...
        public virtual async Task<ResponseDto<string>> DeleteList(List<long> idList)
        {
            ...
            {
                var entity = await _userDomainService.QueryByID(idDelete);
                if (entity == null)
                {
                    notFound++;
                    continue;
                }
                var result = await _userDomainService.DeleteUserById(idDelete);
                if (!string.IsNullOrEmpty(result))
                {
                    return Fail("删除失败");
                }
        ...
        {
            if (string.IsNullOrWhiteSpace(req.NewPassword) || string.IsNullOrEmpty(req.OldPassword))
            {
                return Fail("密码不能为空!");
            }
            var entity = await _userDomainService.QueryByID(req.UserId);
            ...
            var oldPwd = SecurityHelper.EncryptUserPassword(req.OldPassword, entity.Salt);
            ...
            entity.Salt = salt;
            entity.Password = SecurityHelper.EncryptUserPassword(req.NewPassword, salt);
            var result = await _userDomainService.Update(entity);
            if (result)
            {
                return Success("");
        ...
        /// <param name="req"></param>
        /// <returns></returns>
        public async Task<ResponseDto<string>> ResetPwdByAdmin(ResetUserPwdByAdminReq req)
        {
            if (UserId != 1)
            {
                return Fail("只有管理员有权限重置密码!");
            }
