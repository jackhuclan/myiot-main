...
namespace VgAutoDrill.Admin.Repository.Repository
{
    public class BaseRepository<T> : IBaseRepository<T> where T : class, new()
    {
        ...
        {
            return await DBClient.Deleteable<T>(list).ExecuteCommandAsync() > 0 ? true : false;
        }
        ...
        {
            return await DBClient.Updateable<T>().SetColumns(columns).Where(where).ExecuteCommandHasChangeAsync();
        }

        public async Task<bool> UpdateNav(T model, Expression<Func<T, object>> expression)
        {
            return await DBClient.UpdateNav(model).Include(expression).ExecuteCommandAsync();
        }

        public async Task<bool> BulkUpdate(List<T> list)
        {
            return await DBClient.Updateable<T>(list).ExecuteCommandHasChangeAsync();
        }
        ...
        public async Task<IPageList<T>> QueryPageAsync(Expression<Func<T, bool>> predicate, Expression<Func<T, object>> orderByExpression, OrderByType orderByType, int pageIndex = 1, int pageSize = 10)
        {
            ...
            if (pageCount < pageIndex && (data == null || data.Count == 0))
            {
                pageIndex = pageCount;
                data = await DBClient.Queryable<T>().WhereIF(predicate != null, predicate).OrderByIF(orderByExpression != null, orderByExpression, orderByType).ToPageListAsync(pageIndex, pageSize, totalCount);
            }
        ...
        public async Task<IPageList<dynamic>> QueryPageAsync(string sql, string whereSql, object parameters, string orderBy, int pageNum = 1, int pageSize = 10)
        {
            ...
            int pageCount = pageSize > 0 ? (int)Math.Ceiling((double)totalCount / pageSize) : 0;
            if (pageCount < pageNum && (data == null || data.Count == 0))
            {
                pageNum = pageCount;
                data = await DBClient.SqlQueryable<dynamic>(sql).Where(whereSql, parameters).OrderBy(orderBy).ToPageListAsync(pageNum, pageSize, totalCount);
            }
