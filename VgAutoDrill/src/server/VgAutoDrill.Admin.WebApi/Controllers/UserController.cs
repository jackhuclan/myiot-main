...
namespace VgAutoDrill.Admin.WebApi.Controllers
{
    ...
    public class UserController : BaseController
    {
        ...
        [ProducesResponseType(typeof(ResponseDto<bool>), 200)]
        [AllowAnonymous]
        public async Task<ActionResult> LoginForDebug(LoginReq loginReq)
        {
            var result = await _userService.LoginForDebug(loginReq);
            return Ok(result);
        }
        /// <summary>
        /// 退出
        ...
        [ProducesResponseType(typeof(ResponseDto<string>), 200)]
        [PermissionAuthorize("system:user:edit")]
        public async Task<ActionResult> PutSimpleInfo([FromBody] UpdateSimpleUserReq req)
        {
            var result = await _userService.UpdateSimple(req);
            return Ok(result);
        }
        /// <summary>
        /// 删除用户信息
        ...
        {
            var result = await _userService.DeleteList(idList);
            return Ok(result);
        }
        /// <summary>
        ///启用或禁用用户
        ...
        /// <returns></returns>
        [HttpPut]
        [Route("ResetPwdByAdmin")]
        [ProducesResponseType(typeof(ResponseDto<string>), 200)]
        [PermissionAuthorize("system:user:resetPwd")]
        public async Task<ActionResult> ResetPwdByAdmin([FromBody] ResetUserPwdByAdminReq req)
        {
            var result = await _userService.ResetPwdByAdmin(req);
            return Ok(result);
        }
        /// <summary>
        ///导入用户
