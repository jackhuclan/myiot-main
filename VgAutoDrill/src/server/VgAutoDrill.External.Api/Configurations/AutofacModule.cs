...
namespace VgAutoDrill.External.WebApi.Configurations
{
    public class AutofacModule : Autofac.Module
    {
        protected override void Load(ContainerBuilder builder)
        {
            ...
            var servicesDllFile = Path.Combine(basePath, "VgAutoDrill.External.Application.dll");
            ...
            var servicesAdminDllFile = Path.Combine(basePath, "VgAutoDrill.Admin.Application.dll");

            var assemblysServices = Assembly.LoadFrom(servicesDllFile);
            builder.RegisterAssemblyTypes(assemblysServices).AsImplementedInterfaces().InstancePerLifetimeScope()
                .PropertiesAutowired(PropertyWiringOptions.AllowCircularDependencies);

            var assemblysAdminServices = Assembly.LoadFrom(servicesAdminDllFile);
            builder.RegisterAssemblyTypes(assemblysAdminServices).AsImplementedInterfaces().InstancePerLifetimeScope()
                .PropertiesAutowired(PropertyWiringOptions.AllowCircularDependencies);
