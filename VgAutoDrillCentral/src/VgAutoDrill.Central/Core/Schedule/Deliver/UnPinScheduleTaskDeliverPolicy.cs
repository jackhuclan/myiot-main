...
internal class UnPinScheduleTaskDeliverPolicy : PinAndUnpinBaseDeliverPolicy, IScheduleTaskDeliverPolicy
{
    private readonly ILogger<UnPinScheduleTaskDeliverPolicy> _logger;
    private readonly IScheduleTaskManager _scheduleTaskManager;
    private readonly IDeviceManager _deviceManager;

    public UnPinScheduleTaskDeliverPolicy(IServiceProvider serviceProvider) : base(serviceProvider)
    {
        _logger = serviceProvider.GetRequiredService<ILogger<UnPinScheduleTaskDeliverPolicy>>();
    ...
    public async Task<AgvAllocationResult> DeliverScheduleTask(DeliverScheduleTaskRequirement scheduleTaskRequirement)
    {
        ...
        var eventRequest = scheduleTaskRequirement.ScheduleTask.EventRequest;

        _logger.LogWarning($"ScheduleDbCheck检查通过，开始分配,current eventTraceId:{eventRequest.TraceId},source device:{eventRequest.DeviceId}");
        ...
        {
            return await UnloadSilo(callerDevice, agvDevice, eventRequest, scheduleTaskRequirement.ScheduleTask);
        }
        else if (convertedBehavior.InteractionSequence == InteractionSequence.LoadOnly)
        {
            return await LoadSilo(callerDevice, agvDevice, eventRequest, scheduleTaskRequirement.ScheduleTask);
        }
        else
        {
    ...
    private async Task<AgvAllocationResult> UnloadSilo(DeviceProxy callerDevice, Agv agvDevice, DeviceEventReportRequest eventRequest, ScheduleTask scheduleTask)
    {
        ...
        agvDevice.TargetLocation = eventRequest.LocationCode;
        ...
        if (scheduleTask.IsMaster != true
            && (!eventRequest.Params.ContainsKey("IsAuxiliary") || !eventRequest.Params["IsAuxiliary"].ToBool()))
        {
        ...
        if (response != null && response.Code == ErrorCodes.Sys.SUCCESS)
        {
            ...
            if (matchedSchedule != null
                && (!matchedSchedule.MasterScheduleId.HasValue || matchedSchedule.MasterScheduleId == 0))
            {
                await _scheduleTaskManager.SpecifyFollowedSchedule(matchedSchedule.Code, agvDevice.DeviceId, eventRequest.TraceId);
            }
    ...
    private async Task<AgvAllocationResult> LoadSilo(DeviceProxy callerDevice, Agv agvDevice, DeviceEventReportRequest eventRequest, ScheduleTask scheduleTask)
    {
        ...
        ScheduleDto? matchedSchedule = null;
        if (scheduleTask.IsMaster != true
            && (!scheduleTask.MasterScheduleId.HasValue || scheduleTask.MasterScheduleId == 0))
        {
            matchedSchedule = await GetMatchedSchedule(eventRequest);
            if (matchedSchedule == null)
            {
            ...
            var callerDeviceRelate = _deviceManager.GetOnlineDevice(eventRequestRelateDevice.DeviceId);
