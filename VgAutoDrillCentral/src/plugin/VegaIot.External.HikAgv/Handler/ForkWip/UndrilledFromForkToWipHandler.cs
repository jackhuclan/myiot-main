...
/// 生料从中转位拉到线边仓
/// </summary>
public class UndrilledFromForkToWipHandler
{
    private readonly ILogger<UndrilledFromForkToWipHandler> _logger;
    ...
    public UndrilledFromForkToWipHandler(ILogger<UndrilledFromForkToWipHandler> logger,
        ...
        HikArrivedRequestEntity status)
    {
        ...
        _logger.LogInformation($"HikAgvHandler - UndrilledFromForkToWipHandler,task:{agvTask.Id},Start Excute Handle==========================\r\n");
        ...
        _logger.LogInformation($"HikAgvHandler - UndrilledFromForkToWipHandler,task:{agvTask.Id},The verification of device and locations has been passed.\r\n");
        ...
        _logger.LogInformation($"HikAgvHandler - UndrilledFromForkToWipHandler,task:{agvTask.Id},End Excute Handle==========================\r\n");
    ...
    {
        _logger.LogInformation($"HikAgvHandler - UndrilledFromForkToWipHandler,开始-执行第一段任务，料仓任务:{agvTask.Id}.\r\n");
        ...
        string message = "生料:中转位->线边仓，离开中转位,下发完成信息";
        string operationName = "UndrilledFromForkToWipHandler";
        ...
        {
            TransferJobId = agvTask.Id,
            Message = $"车辆已到达，离开中转位，结束-执行第一段任务."
        });

        _logger.LogInformation($"HikAgvHandler - UndrilledFromForkToWipHandler,结束-执行第一段任务，料仓任务:{agvTask.Id}.\r\n");
    }
    ...
    {
        _logger.LogInformation($"HikAgvHandler - UndrilledFromForkToWipHandler,开始-执行第二段任务，料仓任务:{agvTask.Id}.\r\n");
        ...
        {
            TransferJobId = agvTask.Id,
            Message = $"车辆已到达，离开【生料】线边仓，结束-执行第二段任务."
        });
        ...
        _logger.LogInformation($"HikAgvHandler - UndrilledFromForkToWipHandler,结束-执行第二段任务，料仓任务:{agvTask.Id}.\r\n");
    }
}
