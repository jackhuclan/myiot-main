...
public class EmptyBoxFromUnpinToForkHandler
{
    private readonly ILogger<EmptyBoxFromUnpinToForkHandler> _logger;
    ...
    public EmptyBoxFromUnpinToForkHandler(ILogger<EmptyBoxFromUnpinToForkHandler> logger,
        ...
        HikArrivedRequestEntity status)
    {
        ...
        _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,task:{agvTask.Id},Start Excute Handle==========================\r\n");
        ...
        _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,task:{agvTask.Id},The verification of device and locations has been passed.\r\n");
        ...
        _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,task:{agvTask.Id},End Excute Handle==========================\r\n");
    ...
    {
        _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,开始-执行第一段任务，料仓任务:{agvTask.Id}.\r\n");
        ...
        //下PIN->叉齿（中转区）。此时已离开上/下PIN，第一段任务结束
            ...
            && startSchedule.EventRequest != null)
        {
            ...
            _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,task:{agvTask.Id},Start Device Set Schedule Finished.\r\n");
            ...
            _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,task:{agvTask.Id},Start Device Invoke Service.\r\n");
            ...
            _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,task:{agvTask.Id},Start Device Add Schedule Log.\r\n");
        }

        await _transferPlanManager.AddTransferJobLog(new AddTranserJobLogRequest
        {
            TransferJobId = agvTask.Id,
            Message = $"车辆已到达，离开下pin机，结束-执行第一段任务."
        });

        _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,结束-执行第一段任务，料仓任务:{agvTask.Id}.\r\n");
    }
    ...
    {
        _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,开始-执行第二段任务，料仓任务:{agvTask.Id}.\r\n");

        //下PIN -> 叉齿（中转区）。此时已离开叉齿，第二段任务结束
            ...
            && endSchedule.EventRequest != null)
        {
            ...
            _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,task:{agvTask.Id},End Device Set Schedule Finished.\r\n");
                ...
                && startSchedule.EventRequest != null)
            {
                ...
                _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,task:{agvTask.Id},End Device Invoke Service.\r\n");
            }
            ...
            _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,task:{agvTask.Id},End Device Add Schedule Log Request.\r\n");
            ...
            _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,task:{agvTask.Id},End Device Update Database agvTask.\r\n");
        }

        await _transferPlanManager.AddTransferJobLog(new AddTranserJobLogRequest
        {
            TransferJobId = agvTask.Id,
            Message = $"车辆已到达，离开叉齿，结束-执行第二段任务."
        });
        ...
        _logger.LogInformation($"HikAgvHandler - EmptyBoxFromUnpinToForkHandler,结束-执行第二段任务，料仓任务:{agvTask.Id}.\r\n");
    }
}
