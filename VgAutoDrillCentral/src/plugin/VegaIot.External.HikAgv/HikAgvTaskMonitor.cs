...
using VegaIot.External.HikAgv.Handler.MointorAgvTask;
...
{
    private readonly HikAgvSchedulerOptions _hikAgvSchedulerOptions;
    ...
    public HikAgvTaskMonitor(ITransferPlanManager transferPlanManager,
        ...
        ISysConfigManager sysConfigManager)
    {
        ...
        _timer = new PeriodicTimer(TimeSpan.FromSeconds(_hikAgvSchedulerOptions.MonitorTaskStatusInterval));
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
            ...
            && await _timer.WaitForNextTickAsync())
        {
            try
            {
                ...
                if (_hikAgvConfig.GetEnableHikAgvTaskMonitor() == false)
                    continue;
                ...
                if (keyList != null)
                {
                    ...
                    var queryTaskStatusUrl = await _hikAgvConfig.GetQueryTaskStatusUrl();
                    var taskStatusResponse = await _httpRequestInvoker.PostAsJsonAsync<QueryTaskStatusRequest, QueryTaskStatusResponse>(queryTaskStatusUrl, sentTaskToHikAgvData);
                ...
                //查询AGV状态
                var queryAgvStatusUrl = await _hikAgvConfig.GetQueryAgvStatusUrl();
