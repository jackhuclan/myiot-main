...
internal class FirstDrilledFromForkToOutsideExecutor : ITransferSiloExecutor
{
    ...
    {
        _logger.LogInformation($"StdAgvScheduler_FirstDrilledFromForkToOutsideExecutor,task:{job.Id}, begin to ExecuteAsync======================\r\n");
        ...
        if (podLotNum == 0)
        {
            ...
            _logger.LogInformation($"StdAgvScheduler_FirstDrilledFromForkToOutsideExecutor,task:{job.Id},料仓{job.SiloCode}的物料{job.InternalLotNo}下熟料时物料{job.InternalLotNo}数量为0.\r\n");
            ...
            || !job.ExternalLotNo.Contains("@"))
        {
            ...
            _logger.LogInformation($"StdAgvScheduler_FirstDrilledFromForkToOutsideExecutor,task:{job.Id},料仓{job.SiloCode}的物料{job.InternalLotNo}下熟料时物料{job.InternalLotNo}没有料号.\r\n");
        ...
        if (job.ExternalLotNo.Split('@')[0] != job.InternalLotNo)
        {
            ...
            _logger.LogInformation($"StdAgvScheduler_FirstDrilledFromForkToOutsideExecutor,task:{job.Id},料仓{job.SiloCode}的物料{job.InternalLotNo}下熟料时物料{job.InternalLotNo}对内料号与对外料号不一致.\r\n");
            return false;
        }
            ...
            && startLocation != null)
        {
            ...
            _logger.LogInformation($"StdAgvScheduler_FirstDrilledFromForkToOutsideExecutor,task:{job.Id},Start location: {startLocation.PositionCode}.\r\n");
        }
        ...
        //首件区域
        var clinkerStoreArray = (await _stdAgvConfig.GetFirstStore()).Split(',');
        foreach (var item in clinkerStoreArray)
        {
                ...
                && !string.IsNullOrEmpty(startSchedule.Code))
            {
                ...
                _logger.LogInformation($"StdAgvScheduler_FirstDrilledFromForkToOutsideExecutor,task:{job.Id},send request data to Std.\r\n");
                ...
                _logger.LogInformation($"StdAgvScheduler_FirstDrilledFromForkToOutsideExecutor,task:{job.Id},StartSchedule{startSchedule.Id}----SetScheduleAsWorking:TraceId{startSchedule.Code}.\r\n");
        ...
        _logger.LogInformation($"StdAgvScheduler_FirstDrilledFromForkToOutsideExecutor,task:{job.Id}: end to ExecuteAsync======================\r\n");
