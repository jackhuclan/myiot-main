...
internal class PinOrUnPinTrackSiloExcutor : ITransferSiloExecutor
{
    private readonly IScheduleTaskManager _scheduleTaskManager;
    private readonly IHttpRequestInvoker _httpRequestInvoker;
    private readonly ILogger<PinOrUnPinTrackSiloExcutor> _logger;
    private readonly ILocationManager _locationManager;
    private readonly ITransferPlanManager _transferPlanManager;
    private readonly StdAgvConfig _stdAgvConfig;

    public PinOrUnPinTrackSiloExcutor(IScheduleTaskManager scheduleTaskManager,
        ...
        StdAgvConfig stdAgvConfig)
    {
        ...
        _logger = loggerFactory.CreateLogger<PinOrUnPinTrackSiloExcutor>();
    ...
    public async Task<bool> Execute(TransferJob job)
    {
        ...
        _logger.LogInformation($"StdAgvScheduler - PinOrUnPinTrackSiloExcutor,task:{job.Id},begin to ExecuteAsync==========================\r\n");
            ...
            && startLocation.TransAGVInnerPoint != null)
        {
            ...
            _logger.LogInformation($"StdAgvScheduler - PinOrUnPinTrackSiloExcutor,task:{job.Id},startLocation:{startLocation.TransAGVInnerPoint}.\r\n");
        }
            ...
            && endLocation.TransAGVInnerPoint != null)
        {
            ...
            _logger.LogInformation($"StdAgvScheduler - PinOrUnPinTrackSiloExcutor,task:{job.Id},endLocation:{endLocation.TransAGVInnerPoint}.\r\n");
        }

        var taskCode = "VegaTask_" + job.Id.ToStr();

        var StdTyp = await _stdAgvConfig.GetStdTyp();

        var sentTaskToStdAgvData = new StdTaskRequest()
        {
            reqCode = taskCode,
            reqTime = "",
            clientCode = "VEGA",
        ...
        var taskType = job.InteractionSequence == VgAutoDrill.Fundation.Iot.Models.InteractionSequence.LoadOnly ? "F05" : "F07";
        var data = new GenAgvSchedulingTaskData()
        {
        ...
        var model = new STDGenAgvSchedulingTaskRequestEntity()
        {
            ...
            taskTyp = taskType,
            data = data,
            //userCallCodePath = new[] { ceshiPoint[2], ceshiPoint[1] },
            //userCallCodePath = new[] { ceshiPoint[1], ceshiPoint[3] },
            userCallCodePath = new[] { job.InteractionSequence == VgAutoDrill.Fundation.Iot.Models.InteractionSequence.LoadOnly ? lineStore : job.StartLocationCode, job.InteractionSequence == VgAutoDrill.Fundation.Iot.Models.InteractionSequence.LoadOnly ? job.EndLocationCode : lineStore },
            //wbCode = "testStation",
        ...
        //var ret = await _httpRequestInvoker.PostAsJsonAsync<StdTaskRequest, GenAgvSchedulingTaskResponse>(sendTaskToStdAgvUrl, sentTaskToStdAgvData);
        if (ret != null
            && ret.code == 0
        ...
        {
            _logger.LogInformation($"StdAgvScheduler - PinOrUnPinTrackSiloExcutor,task:{job.Id},send request data to Std.\r\n");
            ...
            _logger.LogInformation($"StdAgvScheduler - PinOrUnPinTrackSiloExcutor,task:{job.Id},startSchedule{startSchedule.Id}----SetScheduleAsWorking:TraceId{startSchedule.Code}.\r\n");
            ...
            _logger.LogInformation($"StdAgvScheduler - PinOrUnPinTrackSiloExcutor,task:{job.Id},endSchedule{endSchedule.Id}---SetScheduleAsWorking:TraceId{endSchedule.Code},fromLocation:{fromLocation},startScheduleId:{startSchedule.Id},toLocation:{toLocation},endScheduleId{endSchedule.Id}.\r\n");
        ...
        _logger.LogInformation($"StdAgvScheduler - PinOrUnPinTrackSiloExcutor,task:{job.Id},end to ExecuteAsync==========================\r\n");
