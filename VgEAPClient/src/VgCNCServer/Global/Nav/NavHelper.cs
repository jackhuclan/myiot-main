    ...
    private void Initialization()
    {
        _navList.ForEach(nav =>
        {
            ...
            if (nav.Children is not null)
            {
                ...
                nav.Children.ToList().ForEach(child =>
                {
        ...
        SameLevelNavs.Where(nav => nav.Href is not null).ToList().ForEach(nav =>
        {
            // The following path will not open a new tab
            if (nav.Href is "app/user/view" or "app/user/edit" or "app/vega/details")
            {
                nav.Target = "Self";
            }
