...
public partial class BrokenList
{
    [Inject]
    ...
    private readonly List<DataTableHeader<usrToolsBroken>> dataTableHeaders = new List<DataTableHeader<usrToolsBroken>>
    {
      new (){Text= "机台编号",Align= DataTableHeaderAlign.Start,Sortable= false,Value= nameof(usrToolsBroken.sEquipmentID)},
      new (){ Text= "日期", Value= nameof(usrToolsBroken.dtDate)},
      new (){ Text= "断刀时间", Value= nameof(usrToolsBroken.sTime)},
      new (){ Text= "钻带文件", Value= nameof(usrToolsBroken.sActProgram)},
      new (){ Text= "断刀次数", Value= nameof(usrToolsBroken.sBrokens)},
      new (){ Text= "断刀轴号", Value= nameof(usrToolsBroken.sSpindle)},
      new (){ Text= "刀具号", Value= nameof(usrToolsBroken.sToolID)},
      new (){ Text= "刀径", Value= nameof(usrToolsBroken.sToolDIA)},
      new (){ Text= "孔号", Value= nameof(usrToolsBroken.sHoleID)},
      new (){ Text= "断刀寿命", Value= nameof(usrToolsBroken.sBrokenLife)},
      new (){ Text= "X坐标", Value= nameof(usrToolsBroken.sX)},
      new (){ Text= "Y坐标", Value= nameof(usrToolsBroken.sY)},
      new (){ Text= "程序块", Value= nameof(usrToolsBroken.sProgramBlock)},
      new (){ Text= "程序阶级", Value= nameof(usrToolsBroken.sProgramStep)}
    };

    private List<usrToolsBroken> itemSource = new();
    ...
    private async Task PerformQuery()
    {
        ...
        using (var context = contextFactory.CreateDbContext())
        {
            IQueryable<usrToolsBroken> userQuery = context.usrToolsBrokens.Where(s => s.dtDate >= StartDate && s.dtDate <= EndDate);
    ...
    {
        string path = Path.Combine(AppContext.BaseDirectory, $"Broken-{DateTime.Now.ToString("yyyyMMdd")}.xlsx");
        Workbook book = new Workbook();
        Worksheet sheet = book.Worksheets[0];
        sheet.Cells[0, 0].PutValue("设备编号");
        sheet.Cells[0, 1].PutValue("日期");
        sheet.Cells[0, 2].PutValue("断刀时间");
        ...
        sheet.Cells[0, 9].PutValue("断刀寿命");
        sheet.Cells[0, 10].PutValue("X坐标");
        sheet.Cells[0, 11].PutValue("Y坐标");
        sheet.Cells[0, 12].PutValue("程序块");
        sheet.Cells[0, 13].PutValue("程序阶级");
        int i = 0;
        foreach (usrToolsBroken v in itemSource)
        {
            i++;
            ...
            sheet.Cells[i, 9].PutValue(v.sBrokenLife);
            sheet.Cells[i, 10].PutValue(v.sX);
            sheet.Cells[i, 11].PutValue(v.sY);
            sheet.Cells[i, 12].PutValue(v.sProgramBlock);
            sheet.Cells[i, 13].PutValue(v.sProgramStep);
        }
