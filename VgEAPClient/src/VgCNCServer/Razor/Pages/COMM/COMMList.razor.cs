...
namespace VgCNCServer.Razor.Pages.COMM;

public partial class COMMList
{
    ...
    private readonly List<DataTableHeader<usrCOMM>> dataTableHeaders = new List<DataTableHeader<usrCOMM>>
    {
      new (){Text= "机台编号",Align= DataTableHeaderAlign.Start,Sortable= false,Value= nameof(usrCOMM.sEquipmentID)},
      new (){ Text= "日期", Value= nameof(usrCOMM.dtDate)},
      new (){ Text= "输入时间", Value= nameof(usrCOMM.sTime)},
      new (){ Text= "指令代码", Value= nameof(usrCOMM.sCOMM)},
      new (){ Text= "钻带文件", Value= nameof(usrCOMM.sActProgram)},
      new (){ Text= "事件代码", Value= nameof(usrCOMM.sEventCode)}
    };

    private List<usrCOMM> itemSource = new();
    ...
    private async Task PerformQuery()
    {
        ...
        using (var context = contextFactory.CreateDbContext())
        {
            IQueryable<usrCOMM> userQuery = context.usrCOMMs.Where(s => s.dtDate >= StartDate && s.dtDate <= EndDate);
