...
public partial class ShiftFinalDutyListNew
{
    ...
    private readonly List<DataTableHeader<usrShiftFinalDuty>> _headers = new List<DataTableHeader<usrShiftFinalDuty>>
    {
      new (){Text= "机台编号",Align= DataTableHeaderAlign.Start,Sortable= true,Value= nameof(usrShiftFinalDuty.sEquipmentID)},
      new (){Text= "方式", Value= nameof(usrShiftFinalDuty.sZ)},
      new (){ Text= "日期", Value= nameof(usrShiftFinalDuty.dtDate)},
      new (){ Text= "采集时间", Value= nameof(usrShiftFinalDuty.sTime)},
      new (){ Text= "稼动率", Value= nameof(usrShiftFinalDuty.sDuty)},
      new (){ Text= "轴数", Value= nameof(usrShiftFinalDuty.sT)},
      new (){ Text= "加工时间", Value= nameof(usrShiftFinalDuty.sWorkTime)},
      new (){ Text= "等待时间", Value= nameof(usrShiftFinalDuty.sWaitTime)},
      new (){ Text= "停机时间", Value= nameof(usrShiftFinalDuty.sStopTime)},
      new (){ Text= "总时间", Value= nameof(usrShiftFinalDuty.sTotalTime)},
      new (){ Text= "孔数", Value= nameof(usrShiftFinalDuty.sHits)},
      new (){ Text= "班次总孔数", Value= nameof(usrShiftFinalDuty.sShiftHits)},
      new (){ Text= "断刀", Value= nameof(usrShiftFinalDuty.sBrokens)},
      new (){ Text= "换板次数", Value= nameof(usrShiftFinalDuty.sChangePanels)},
      new (){ Text= "换料次数", Value= nameof(usrShiftFinalDuty.sChangeDrills)},
      new (){ Text= "换料时间", Value= nameof(usrShiftFinalDuty.sChangeDrillsTime)},
      //new (){ Text= "注册日期", Value= nameof(usrShiftDuty.sRegistrationDate)},
      new (){ Text= "所属班次", Value= nameof(usrShiftFinalDuty.sShift)}
    };

    private List<usrShiftFinalDuty> _desserts = new List<usrShiftFinalDuty>
    {
    };
    ...
    private async Task PerformQuery()
    {
        ...
        using (var context = contextFactory.CreateDbContext())
        {
            IQueryable<usrShiftFinalDuty> userQuery = context.usrShiftFinalDutys.Where(s => s.dtDate >= StartDate && s.dtDate <= EndDate);
    ...
    private async Task ExportExcel()
    {
        ...
        int i = 0;
        foreach (usrShiftFinalDuty v in _desserts)
        {
            i++;
