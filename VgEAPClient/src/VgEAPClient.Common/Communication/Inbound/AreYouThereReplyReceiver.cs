...
public class AreYouThereReplyReceiver
{
    private readonly IEQPDataReceiver _eQPDataReceiver;
    private readonly ILogger<AreYouThereReplyReceiver> _logger;
    private readonly EAPClientOptions _eAPClientOptions;
    public AreYouThereReplyReceiver(ILogger<AreYouThereReplyReceiver> logger,
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        var faildata = new AreYouThereReplyModel()
        {
        ...
        try
        {
            ...
            _logger.LogInformation($@"AreYouThereReply[EAP -> EQP] - {content}");

            var redata = JsonSerializer.Deserialize<AreYouThereReplyModel>(content,
            new JsonSerializerOptions
            {
                PropertyNamingPolicy = null
            });
            AreYouThereReplyModel senddata = new AreYouThereReplyModel();
            if (redata != null)
            {
                senddata = await _eQPDataReceiver.AreYouThereReply(redata);
            }
            else
            {
                senddata = faildata;
            ...
            _logger.LogInformation($@"AreYouThereReply[EQP -> EAP] - {senddata.ToJson()}");
            await context.Response.WriteAsJsonAsync(senddata);
        }
        catch (Exception ex)
        {
            _logger.LogInformation($@"AreYouThereReply[error] - {ex.Message}");
