...
public class DateTimeCommandReceiver
{
    private readonly IEQPDataReceiver _eQPDataReceiver;
    private readonly ILogger<DateTimeCommandReceiver> _logger;
    private readonly EAPClientOptions _eAPClientOptions;
    public DateTimeCommandReceiver(ILogger<DateTimeCommandReceiver> logger,
    ...
    {
        var faildata = new DateTimeCommandModel()
        {
        ...
        try
        {
            ...
            _logger.LogInformation($@"DateTimeCommand[EAP -> EQP] - {content}");

            var redata = JsonSerializer.Deserialize<DateTimeCommandModel>(content,
            new JsonSerializerOptions
            {
                PropertyNamingPolicy = null
            });
            var senddata = new DateTimeCommandModel();
            if (redata != null)
            {
                senddata = await _eQPDataReceiver.DateTimeCommand(redata);
            }
            else
            {
                senddata = faildata;
            ...
            _logger.LogInformation($@"DateTimeCommand[EQP -> EAP] - {senddata.ToJson()}");
            await context.Response.WriteAsJsonAsync(senddata);
        }
        catch (Exception ex)
        {
            _logger.LogInformation($@"DateTimeCommand[error] - {ex.Message}");
