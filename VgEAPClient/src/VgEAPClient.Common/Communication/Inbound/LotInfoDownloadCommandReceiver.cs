...
public class LotInfoDownloadCommandReceiver
{
    private readonly IEQPDataReceiver _eQPDataReceiver;
    private readonly ILogger<LotInfoDownloadCommandReceiver> _logger;
    private readonly EAPClientOptions _eAPClientOptions;

    public LotInfoDownloadCommandReceiver(ILogger<LotInfoDownloadCommandReceiver> logger,
    ...
    {
        var faildata = new LotInfoDownloadCommandModel()
        {
        ...
        try
        {
            ...
            _logger.LogInformation($@"LotInfoDownloadCommand[EAP -> EQP] - {content}");

            var redata = JsonSerializer.Deserialize<LotInfoDownloadCommandModel>(content,
            new JsonSerializerOptions
            {
                PropertyNamingPolicy = null
            });
            var senddata = new LotInfoDownloadCommandModel();
            if (redata != null)
            {
                senddata = await _eQPDataReceiver.LotInfoDownloadCommand(redata);
            }
            else
            {
                senddata = faildata;
            ...
            _logger.LogInformation($@"LotInfoDownloadCommand[EQP -> EAP] - {senddata.ToJson()}");
            await context.Response.WriteAsJsonAsync(senddata);
        }
        catch (Exception ex)
        {
            _logger.LogInformation($@"LotInfoDownloadCommand[error] - {ex.Message}");
