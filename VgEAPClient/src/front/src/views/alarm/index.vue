  ...
  <div class="app-container">
    <el-card class="page-header">
      ...
      >
        ...
        <el-form-item prop="sAlarmID">
          <border-title>
            <el-input
              style="border: none; width: 200px"
              v-model="queryParams.sAlarmID"
              placeholder="报警指令"
              clearable
              @keyup.enter.native="handleQuery"
          /></border-title>
        </el-form-item>
        <el-form-item prop="sAlarmDesc">
          <border-title>
            <el-input
              style="border: none; width: 200px"
              v-model="queryParams.sAlarmDesc"
              placeholder="报警说明"
              clearable
              @keyup.enter.native="handleQuery"
          /></border-title>
    ...
    <el-card class="page-body">
      <el-table :ref="page" v-loading="loading" :data="list" border>
        ...
        <el-table-column label="开始时间" align="center"
          ><template slot-scope="scope">{{
            parseTime(scope.row.sStartTime, " {h}:{i}:{s}")
          }}</template>
        </el-table-column>
...
<script>
import { queryAlarmList } from "@/api/alarm";
export default {
  data() {
    return {
      page: "alarm",
  ...
  methods: {
    ...
    getList() {
      ...
      this.loading = true;
      this.loading = true;
      queryAlarmList(this.queryParams).then((res) => {
        this.list = res.List;
